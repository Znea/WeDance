<x-app-layout>
    ...
    <div id="modal-profesor" tabindex="-1" class="fixed inset-0 z-40 hidden flex items-center justify-center overflow-y-auto">
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <div>
                        <form action="{{route('clases.asignarProfesor', ['clase' => $clase->id])}}" method="POST" class="w-10/12 mx-auto text-start">
                            @csrf
                            @method('PUT')

                            @foreach ($profesores as $p )
                                <p class="text-xs mb-2"><input type="radio" value="{{$p->id}}" name="teacher" class="ms-2 me-2"> {{$p->name}} {{$p->lastname}} </p>
                            @endforeach
    ...
    <div class="contenedor w-5/6 mx-auto">
        ...
        <div class="show">
            <div class="text-center">
                <p class="md:text-md">{{$clase->description}}</p>
            </div>

            <div class="md:flex md:space-between mt-10 ">
                <div class="w-full mx-auto text-center horario">
                    <h2>HORARIO</h2>
                    <p class="mt-5 text-xs ">{{$clase->schedule}}</p>
                </div>
                <div class="w-full mx-auto text-center profesor">
                    <h2 class="mb-5">PROFESOR</h2>
                    ...
                    @if ($clase->teacher == null)
                        <div class="w-full md:w-1/2 mx-auto mt-5">
                            <button class="bg-secondary login" id="asignarProfesor">ASIGNAR PROFESOR</button>
                        </div>
                    @endif
                </div>
            </div>
